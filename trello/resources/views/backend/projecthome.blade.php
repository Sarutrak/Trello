              ...
              @foreach($project as $pj)
               ...
               <!-- Card -->
                  <div class="card text-white  mb-3 shadow cardproject">
                        <form method="POST" action="{{ route('projecthome.destroy', $pj->id) }}">
                           {!! method_field('DELETE') !!}
                           @csrf
                        <div class="card-header btnmargin"><i class="fas fa-folder-open" style="margin-right: 10px;"></i>{{$pj->project_name}}
                           <button type="close" class="btn btn-danger btn-circle" style="float: right; font-size: 13px; "><i class="fa fa-times" style="text-align: center;"></i></button>
                        </form> 
                        </div> <!-- head -->
                        <div class="card-body">
